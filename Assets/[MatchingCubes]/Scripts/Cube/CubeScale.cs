...
public class CubeScale : MonoBehaviour
{
    ...
    private void OnMatched() 
    {
        ...
        matchSequence.Append(_scaleBody.DOPunchScale(Vector3.one * PUNCH_STRENGTH, PUNCH_DURATION, vibrato: 1).SetId(_punchTweenID).SetEase(PUNCH_EASE))
        .Append(_scaleBody.DOScale(_defaultScale * MIN_SCALE_MULTIPLIER, SCALE_DURATION).SetEase(SCALE_EASE).SetId(_scaleTweenID)).OnComplete(() => Destroy(gameObject));
    }
    ...
    {
        KillTweens();
        _scaleBody.DOPunchScale(Vector3.one * PUNCH_STRENGTH, PUNCH_DURATION, vibrato: 1).SetId(_punchTweenID).SetEase(PUNCH_EASE);
    }
